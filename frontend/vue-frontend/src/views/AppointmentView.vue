<template>
  <div>
    ...
    <div class="container mt-4">
      ...
      <div style="max-width: 800px;" class="border p-4 mx-auto">
        ...
        <form class="form" @submit.prevent="saveChanges">
          ...
          <div>
            <button @click="deleteAppointment" class="btn btn-primary btn-danger">Delete</button>
          </div>
        </form>
...
export default {
  ...
  methods: {
    async saveChanges() {
      ...
      console.log(token);
      if (token) {
        await axios.post('http://127.0.0.1:5000/appointments/update', this.appointment, {
          headers: {
            Authorization: `${token}`
          }
        })
    ...
    async deleteAppointment() {
      const token = localStorage.getItem('token');
      if (token) {
        await axios.delete(`http://127.0.0.1:5000/api/appointments/delete/${this.$route.params.id}`, {
          headers: {
            Authorization: `${token}`
          }
        })
